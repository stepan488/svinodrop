...
const { MARKET_ITEMS, CASES, dropWeight } = require('./seed') as typeof import('./seed')
...
async function main() {
  ...
  const existing = await api('/api/admin/items', token) as Array<{ id: string }>
  ...
  const existingCases = await api('/api/admin/cases', token) as Array<{ id: string; slug: string }>
  for (const config of CASES) {
    const targetCase = existingCases.find((item) => item.slug === config.slug)
    if (!targetCase) throw new Error(`Case ${config.slug} does not exist; run the regular seed when direct TLS is available.`)
    ...
    const items = MARKET_ITEMS.filter((item) => (config.itemIds as readonly string[]).includes(item.id)).map((item) => ({ itemId: item.id, weight: dropWeight(item.price) }))
