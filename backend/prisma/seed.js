...
exports.CASES = exports.MARKET_ITEMS = exports.BASE_MARKET_ITEMS = void 0;
exports.dropWeight = dropWeight;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
// Curated SvinoDrop catalogue supplied by the owner. Prices are stored in kopecks.
exports.BASE_MARKET_ITEMS = [
...
exports.MARKET_ITEMS = exports.BASE_MARKET_ITEMS;
exports.CASES = [
    { name: 'Генста Свин!', slug: 'gensta-svin', price: 49900, image: 'https://i.ibb.co/gb4r4CLF/b4f6cb58-752e-44ae-885c-bbbe73098ba9.png', collection: 'Свиноохотники', itemIds: ['1', '2', '3', '4', '5', '6', '7', '8', '9'], weights: { '1': 12, '2': 15, '3': 22, '4': 28, '5': 35, '6': 45, '7': 65, '8': 320, '9': 420 } },
    ...
    { name: 'Богатый Свин', slug: 'rich-pig', price: 499900, image: 'https://i.ibb.co/nsxsNwr7/fc659d53-b9c9-4f72-8e58-e65a50cc7d80.png', collection: 'Свинячий Окуп', itemIds: ['21', '22', '23', '24', '25', '26'], weights: {} },
    { name: 'Мажор Свин', slug: 'major-pig', price: 799900, image: 'https://i.ibb.co/zHtrBHbp/4949a980-100f-4a60-ba88-7617985c3a74.png', collection: 'Свинячий Окуп', itemIds: ['23', '24', '25', '26', '27'], weights: {} },
    { name: 'Миллионер Свин', slug: 'millionaire-pig', price: 1499900, image: 'https://i.ibb.co/QFjc9f5x/7873dd36-8b67-44ac-9219-a7daf275efb3.png', collection: 'Свинячий Окуп', itemIds: ['25', '26', '27', '28', '29'], weights: {} },
    { name: 'Миллиардер Свин', slug: 'billionaire-pig', price: 3499900, image: 'https://i.ibb.co/SXLJzPXt/c7760d81-66b9-4df6-a8ab-a21f573f6474.png', collection: 'Свинячий Окуп', itemIds: ['27', '28', '29', '30', '31', '32'], weights: {} },
];
function dropWeight(price) {
...
async function main() {
    for (const item of exports.MARKET_ITEMS) {
    ...
    await prisma.item.updateMany({ where: { id: { notIn: exports.MARKET_ITEMS.map((item) => item.id) } }, data: { active: false } });
    await prisma.inventory.updateMany({ where: { itemId: { notIn: exports.MARKET_ITEMS.map((item) => item.id) }, removedAt: null }, data: { removedAt: new Date() } });
    for (const config of exports.CASES) {
        ...
        const contents = exports.MARKET_ITEMS.filter((item) => config.itemIds.includes(item.id));
        await prisma.$transaction([
            prisma.caseItem.deleteMany({ where: { caseId: caseData.id } }),
            prisma.caseItem.createMany({ data: contents.map((item) => ({ caseId: caseData.id, itemId: item.id, weight: config.weights?.[item.id] || dropWeight(item.price) })) }),
        ]);
    }
    console.log(`SvinoDrop: updated ${exports.MARKET_ITEMS.length} live skins and ${exports.CASES.length} case collections.`);
}
