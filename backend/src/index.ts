...
function validFleet(cells: unknown): cells is Cell[] {
  ...
  const sizes: number[] = [];
  for (const group of fleetGroups(result)) { const sameX = group.every((cell) => cell.x === group[0].x); const sameY = group.every((cell) => cell.y === group[0].y); if ((!sameX && !sameY) || group.length > 5) return false; sizes.push(group.length); }
