...
type Skin = { id: string; name: string; wear: string; price: number; image: string; rarity: string; upgradeEligible?: boolean }
...
type RoadGame = { status: RoadStatus; wager: number; steps: number[]; multiplier: number; payout: number; last?: { choice: number; safe: boolean }; maxSteps: number }
...
type SiteSound = 'case' | 'reveal' | 'upgrade' | 'win' | 'sell' | 'contract' | 'shot' | 'hit' | 'card' | 'cashout'
const playSiteSound = (sound: SiteSound) => window.dispatchEvent(new CustomEvent<SiteSound>('svino-sound', { detail: sound }))
const soundTones: Record<SiteSound, number[]> = {
  case: [180, 240, 320], reveal: [420, 560, 760], upgrade: [220, 330, 440], win: [520, 660, 880], sell: [580, 740],
  contract: [170, 230, 310, 410], shot: [145, 90], hit: [110, 180, 90], card: [360, 520], cashout: [480, 640, 920],
}
function playSynthSound(sound: SiteSound) {
  ...
  if (sound === 'case' || sound === 'upgrade') {
    ...
    oscillator.frequency.exponentialRampToValueAtTime(sound === 'upgrade' ? 680 : 390, now + duration * .82)
    oscillator.frequency.exponentialRampToValueAtTime(sound === 'upgrade' ? 300 : 220, now + duration)
    gain.gain.setValueAtTime(.0001, now)
    gain.gain.exponentialRampToValueAtTime(.045, now + .04)
    gain.gain.setValueAtTime(.038, now + duration * .72)
  ...
  soundTones[sound].forEach((frequency, index) => {
    ...
    gain.gain.exponentialRampToValueAtTime(.075, now + index * .075 + .012)
    gain.gain.exponentialRampToValueAtTime(.0001, now + index * .075 + .11)
    oscillator.connect(gain).connect(context.destination); oscillator.start(now + index * .075); oscillator.stop(now + index * .075 + .12)
  })
  window.setTimeout(() => void context.close(), sound === 'upgrade' ? 3900 : sound === 'case' ? 2700 : 700)
}
...
export default function App() {
  const [page, setPage] = useState<'cases' | 'games' | 'upgrade' | 'battles' | 'naval' | 'mines' | 'road' | 'contract' | 'crash' | 'giveaways' | 'inventory' | 'profile' | 'chat' | 'leaderboard' | 'admin'>('cases')
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => localStorage.getItem('svino-sound') !== 'off')
  ...
  useEffect(() => { localStorage.setItem('svino-sound', soundEnabled ? 'on' : 'off') }, [soundEnabled])
  useEffect(() => {
    const handleSound = (event: Event) => { if (soundEnabled) playSynthSound((event as CustomEvent<SiteSound>).detail) }
  ...
  const login = async (email: string, password: string, username?: string) => {
    ...
    localStorage.setItem('svino-token', data.token); setToken(data.token); setUser(data.user); setAuthOpen(false)
  ...
  const finishUpgradeAnimation = async () => { if (upgradePhase !== 'spinning' || !upgradeResult) return; playSiteSound(upgradeResult.success ? 'win' : 'reveal'); setUpgradePhase('result'); setUpgradeBalance(0); setSources([]); setTarget(null); await request(`/api/upgrades/${upgradeResult.upgradeId}/reveal`, token, { method: 'POST' }); await refreshPrivate() }
  ...
  return <main className="app-shell">
    ...
    <header className="topbar">
      ...
      <div className="top-actions"><button className={`sound-toggle ${soundEnabled ? 'on' : ''}`} onClick={() => { if (!soundEnabled) playSynthSound('reveal'); setSoundEnabled((enabled) => !enabled) }} aria-label={soundEnabled ? 'Выключить звуки' : 'Включить звуки'}>{soundEnabled ? '🔊' : '🔇'}</button><span className="online"><i /> {online} свинок онлайн</span>{user ? <><button className="balance-topup" onClick={() => setPage('profile')}>＋ Пополнить баланс</button><button className="user-chip" onClick={() => setPage('profile')}><span className="avatar">🐽</span><b>{coins(user.balance)} <small>SC</small></b><em>{user.username}</em></button></> : <button className="login" onClick={() => setAuthOpen(true)}>Войти</button>}</div>
    </header>
    ...
    {page === 'upgrade' && <section className="page upgrade-page">
      ...
      <div className="upgrade-board">
        ...
        <UpgradeColumn title="ЦЕЛЬ" subtitle="Предмет, который получишь при успехе" selectedSkins={target ? [target] : []} selectedCaption="Твоя цель">{skins.filter((skin) => skin.upgradeEligible !== false && skin.price > upgradeStake).map((skin) => <SkinCard key={skin.id} skin={skin} selected={target?.id === skin.id} onClick={() => { setTarget(skin); setUpgradePhase('idle'); setUpgradeResult(null) }} />)}</UpgradeColumn>
      </div>
    </section>}
    ...
    {page === 'road' && <PigRoadPage token={token} user={user} onRequireAuth={() => setAuthOpen(true)} onBalance={refreshPrivate} toast={toast}/>}
    ...
    {onboardingOpen && user && <OnboardingModal username={user.username} onClose={() => { localStorage.setItem(`svino-onboarding:${user.id}`, 'seen'); setOnboardingOpen(false) }} onStart={() => { localStorage.setItem(`svino-onboarding:${user.id}`, 'seen'); setOnboardingOpen(false); setPage('cases') }}/>}
...
const roadPlatformLabels = ['Сочный шорткат', 'Золотой рельс', 'Свинский мост', 'Бекон-экспресс']
const roadPigImage = 'https://i.ibb.co/yBRpJjPv/ae3fd723-35f3-40aa-9ac4-3703fd5c274f.png'

function PigRoadPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => Promise<void>; toast: (text: string) => void }) {
  ...
  const nextMultiplier = playing ? roadMultipliers[game.steps.length] || 48 : roadMultipliers[0]
  const load = async () => { if (!token) { setGame(null); return }; try { const data = await request('/api/road', token); setGame(data.game); setRoundKey((key) => key + 1) } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось загрузить Свинорейс') } }
  useEffect(() => { void load() }, [token])
  const start = async () => {
    if (!user) return onRequireAuth()
    if (stakeCoins < 500 || stakeCoins > 555000) return toast('Ставка — от 500 до 555 000 SC')
    setBusy(true)
    try { const data = await request('/api/road/start', token, { method: 'POST', body: JSON.stringify({ wager: stakeCoins * 100 }) }); setGame(data.game); setRoundKey((key) => key + 1); await onBalance(); toast('Свинья на старте. Выбери платформу!') } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось начать Свинорейс') } finally { setBusy(false) }
  }
  const step = async (choice: number) => {
    if (!playing || busy) return
    setBusy(true)
    try { const data = await request('/api/road/step', token, { method: 'POST', body: JSON.stringify({ choice }) }); setGame(data.game); setRoundKey((key) => key + 1); playSiteSound(data.game.last?.safe ? 'card' : 'hit'); await onBalance(); if (!data.game.last?.safe) toast('Платформа была заминирована — ставка сгорела.'); if (data.game.status === 'WON') toast(`Свинья добралась до финиша: ${coins(data.game.payout)} SC!`) } catch (error) { toast(error instanceof Error ? error.message : 'Шаг не выполнен') } finally { setBusy(false) }
  }
  const cashout = async () => {
    if (!playing || !game.steps.length) return
    setBusy(true)
    try { const data = await request('/api/road/cashout', token, { method: 'POST' }); setGame(data.game); playSiteSound('cashout'); await onBalance(); toast(`Забрано ${coins(data.game.payout)} SC!`) } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось забрать выигрыш') } finally { setBusy(false) }
  }
  const resolvedChoice = game?.last?.choice
  return <section className="page compact-page road-page"><header className="road-hero"><div><p className="eyebrow">PIGGY ROAD · СРЕДНИЙ РЕЖИМ</p><h1>Свиная <strong>дорога</strong></h1><p>Проведи свинью по четырём платформам и забери выигрыш до того, как дорога оборвётся.</p></div><button onClick={() => setRulesOpen((open) => !open)}>Как играть? {rulesOpen ? '⌃' : '⌄'}</button></header>{rulesOpen && <div className="road-rules"><b>Только средняя сложность · до ×48.</b><span>Поставь 500–555 000 SC, выбери одну из четырёх платформ и забери выигрыш после любого безопасного шага. Свинья либо проходит дальше, либо забег сразу заканчивается.</span></div>}<div className="road-game"><section className={`road-stage ${game?.status?.toLowerCase() || 'idle'}`}><div className="road-stage-top"><span>🐷 ПЛАТФОРМА {playing ? game.steps.length + 1 : game?.status === 'WON' ? game.maxSteps : 1}/{game?.maxSteps || 12}</span><strong>{playing ? 'ВЫБЕРИ СЛЕДУЮЩУЮ ПЛАТФОРМУ' : game?.status === 'LOST' ? 'СВИНЬЯ СНЯТА С ДИСТАНЦИИ' : game?.status === 'WON' ? 'ДОРОГА ПРОЙДЕНА!' : 'ДОРОГА ЖДЁТ СВИНЬЮ'}</strong></div><div className="road-tracks"><span/><span/><span/></div><div className="road-pig-lane"><img className="road-pig" src={roadPigImage} alt="Свинья на старте"/><div className="road-finish">🏁</div></div><div className="road-platform-strip" key={roundKey}>{roadPlatformLabels.map((label, index) => { const selected = resolvedChoice === index; const state = selected ? (game?.last?.safe ? 'safe' : 'mine') : ''; return <button disabled={!playing || busy} onClick={() => step(index)} className={`road-platform ${state}`} key={label}><span className="road-platform-grate">{selected ? game?.last?.safe ? '🐷' : '💣' : '✦'}</span><b>{label}</b><em>{multiplier(nextMultiplier)}</em></button> })}</div><div className="road-stage-footer"><span>Текущий множитель <b>{multiplier(game?.multiplier || 1)}</b></span><span>Следующий шаг <b>{multiplier(nextMultiplier)}</b></span></div></section><aside className="road-controls"><p className="eyebrow">СВИНОСТАВКА</p><h2>{playing ? 'Прыгай аккуратно' : game?.status === 'LOST' ? 'Попробовать снова' : game?.status === 'WON' ? 'Дорога покорена' : 'Подготовь забег'}</h2><label>Сумма ставки, SC<input type="number" min="500" max="555000" disabled={playing} value={stake} onChange={(event) => setStake(event.target.value)}/></label><div className="road-quick"><button disabled={playing} onClick={() => setStake('500')}>500</button><button disabled={playing} onClick={() => setStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 200))))}>50%</button><button disabled={playing} onClick={() => setStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 100))))}>MAX</button></div><div className="road-odds"><strong>Максимум {multiplier(48)}</strong><small>Каждая новая платформа повышает твой выигрыш.</small></div>{playing && game.steps.length > 0 && <div className="road-cashout-preview">Сейчас можно забрать <b>{coins(Math.floor(game.wager * game.multiplier))} SC</b></div>}<button className={`pig-button road-action ${playing ? 'cashout' : ''}`} disabled={busy || (playing && !game.steps.length)} onClick={playing ? cashout : start}>{busy ? 'СВИНЬЯ ДУМАЕТ…' : playing ? `ЗАБРАТЬ ${coins(Math.floor(game.wager * game.multiplier))} SC →` : `НАЧАТЬ ЗА ${coins(stakeCoins * 100)} SC →`}</button></aside></div></section>
}
...
function ContractPage({ token, user, inventory, onRequireAuth, onBalance, toast }: { token: string; user: User | null; inventory: Inventory[]; onRequireAuth: () => void; onBalance: () => Promise<void>; toast: (text: string) => void }) {
  ...
  const [busy, setBusy] = useState(false)
  const [result, setResult] = useState<{ inventoryId: string; item: Skin; stake: number; multiplier: number } | null>(null)
  ...
  const create = async () => {
    ...
    try {
      ...
      setResult(data); setScratchDone(!riskMode); setMixingItems(selectedItems); setSelected([]); setPhase('mixing'); playSiteSound('contract'); await onBalance(); window.setTimeout(() => { playSiteSound('reveal'); setPhase('result') }, 2400)
    } catch (error) { toast(error instanceof Error ? error.message : 'Контракт не выполнен') } finally { setBusy(false) }
  }
  const keepResult = async () => { try { await request('/api/contracts/reveal', token, { method: 'POST' }); playSiteSound('win'); await onBalance(); setMixingItems([]); setPhase('idle'); setResult(null); toast('Предмет оставлен в инвентаре.') } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось раскрыть предмет') } }
  const sellResult = async () => {
    ...
    try {
      ...
      playSiteSound('sell'); await onBalance(); setMixingItems([]); setPhase('idle'); setResult(null); toast(`Предмет продан за ${coins(data.payout)} SC.`)
...
function CrashPage({ token, user, inventory, onRequireAuth, onBalance, toast }: { token: string; user: User | null; inventory: Inventory[]; onRequireAuth: () => void; onBalance: () => Promise<void>; toast: (text: string) => void }) {
  ...
  const placeBet = async () => {
    if (!user) return onRequireAuth()
    if (stake < 50_000 || stake > 55_500_000) return toast('Общая ставка — от 500 до 555 000 SC.')
    setBusy(true)
  ...
  const cashout = async () => {
    setBusy(true)
    try { const data = await request('/api/crash/cashout', token, { method: 'POST' }); playSiteSound('cashout'); setBet(data.bet); await onBalance(); await reload(); toast(`Забрано ${coins(data.payout)} SC!`) } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось вывести ставку') } finally { setBusy(false) }
  }
  ...
  return <section className="page compact-page crash-page">
    <header className="crash-hero"><div><p className="eyebrow">PIGGY CRASH · SHARED LIVE ROUND</p><h1>Свинский <strong>краш</strong></h1><p>Коэффициент растёт — выведи ставку до краша. Невыведенные свинокоины и скины сгорают.</p></div><button onClick={() => setRulesOpen((open) => !open)}>Как играть? {rulesOpen ? '⌃' : '⌄'}</button></header>
    {rulesOpen && <div className="crash-rules"><b>Ставка 500–555 000 SC.</b><span>Во время 15-секундной подготовки можно поставить балансом, скинами или вместе. После старта вывод доступен в любую секунду, пока свинокраш не остановился.</span></div>}
    <div className="crash-history">{history.length ? history.map((item) => <span className={item.multiplier >= 5 ? 'high' : item.multiplier <= 1.3 ? 'low' : ''} key={item.id}>{multiplier(item.multiplier)}</span>) : <span>История первого раунда собирается…</span>}</div>
    <div className="crash-layout"><section className={`crash-stage ${round?.phase?.toLowerCase() || ''}`}><div className="crash-stage-head"><span>🐷 РАУНД #{round?.id.slice(-5) || '…'}</span><b>{round?.phase === 'BETTING' ? `СТАРТ ЧЕРЕЗ ${seconds.toFixed(1)} сек` : round?.phase === 'RUNNING' ? 'СВИНЬЯ ЛЕТИТ!' : 'БАБАХ! РАУНД ЗАВЕРШЁН'}</b></div><div className="crash-chart"><div className="crash-grid"/><svg viewBox="0 0 620 210" preserveAspectRatio="none"><defs><linearGradient id="crash-fill" x1="0" x2="0" y1="0" y2="1"><stop stopColor="#ff4e9d" stopOpacity=".42"/><stop offset="1" stopColor="#ff4e9d" stopOpacity="0"/></linearGradient></defs><path d={`M38 184 C 170 180, 380 ${Math.min(180, curveY + 66)}, 580 ${curveY}`} fill="none" stroke="#ff65b4" strokeWidth="4"/><path d={`M38 184 C 170 180, 380 ${Math.min(180, curveY + 66)}, 580 ${curveY} L 580 204 L38 204 Z`} fill="url(#crash-fill)"/></svg><div className="crash-multiplier"><span>{round?.phase === 'BETTING' ? 'ГОТОВИМ ВЗЛЁТ' : round?.phase === 'CRASHED' ? 'КРАШ НА' : 'ТЕКУЩИЙ ИКС'}</span><b>{multiplier(round?.phase === 'CRASHED' ? round.crashMultiplier || 1 : current)}</b></div></div>{bet && <div className={`crash-bet-status ${bet.status.toLowerCase()}`}><span>{bet.status === 'CASHED_OUT' ? '🏆' : bet.status === 'LOST' ? '💥' : '🎲'}</span><div><b>{bet.status === 'CASHED_OUT' ? `Вывел на ${multiplier(bet.cashoutMultiplier || 1)}` : bet.status === 'LOST' ? 'Ставка сгорела на краше' : `Ставка ${coins(bet.stake)} SC в полёте`}</b><small>{bet.status === 'BET' ? `Сейчас можно забрать ${coins(bet.livePayout || Math.floor(bet.stake * current))} SC` : bet.status === 'CASHED_OUT' ? `${coins(bet.payout || 0)} SC уже на балансе` : 'Следующий раунд через несколько секунд'}</small></div>{bet.status === 'BET' && round?.phase === 'RUNNING' && <button className="pig-button" disabled={busy} onClick={cashout}>ЗАБРАТЬ →</button>}</div>}</section><aside className="crash-controls"><p className="eyebrow">СВИНОСТАВКА</p><h2>{canBet ? 'Поставь до старта' : round?.phase === 'BETTING' ? 'Ставка принята' : 'Следи за полётом'}</h2><label>Баланс, SC<input type="number" min="0" max="555000" disabled={!canBet} value={balanceStake} onChange={(event) => setBalanceStake(event.target.value)}/></label><div className="crash-quick"><button disabled={!canBet} onClick={() => setBalanceStake('500')}>500</button><button disabled={!canBet} onClick={() => setBalanceStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 200))))}>50%</button><button disabled={!canBet} onClick={() => setBalanceStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 100))))}>MAX</button></div><div className="crash-skins"><span>Скины в ставке <small>необязательно</small></span><div>{inventory.slice(0, 12).map((entry) => <button disabled={!canBet} className={skinIds.includes(entry.id) ? 'chosen' : ''} onClick={() => toggleSkin(entry.id)} key={entry.id}><img src={entry.item.image} alt=""/><em>{coins(entry.item.price)}</em></button>)}</div></div><div className="crash-summary"><span>Баланс <b>{coins(balanceValue)} SC</b></span><span>Скины <b>{coins(skinValue)} SC</b></span><strong>Всего {coins(stake)} SC</strong></div><button className="pig-button crash-action" disabled={!canBet || busy} onClick={placeBet}>{busy ? 'ПРИНИМАЕМ…' : canBet ? 'ПОСТАВИТЬ НА ВЗЛЁТ →' : 'СТАВКИ ЗАКРЫТЫ'}</button></aside></div>
  </section>
}

function GamesHub({ onOpen }: { onOpen: (game: 'upgrade' | 'battles' | 'naval' | 'mines' | 'road' | 'contract' | 'crash') => void }) { const games: Array<{ id: 'upgrade' | 'battles' | 'naval' | 'mines' | 'road' | 'contract' | 'crash'; icon: string; eyebrow: string; title: string; text: string; action: string }> = [{ id: 'upgrade', icon: '🎯', eyebrow: 'RISK IT FOR THE BACON', title: 'Свинский апгрейд', text: 'Собери до 8 скинов, выбери цель и рискни на красивом колесе.', action: 'К апгрейду' }, { id: 'battles', icon: '⚔️', eyebrow: 'CASE BATTLE', title: 'Кейс-баттлы', text: 'Создавай комнаты, зови игроков или свиноботов и забирай весь банк.', action: 'В баттлы' }, { id: 'naval', icon: '⚓', eyebrow: 'PIGGY NAVAL', title: 'Морской бой', text: 'Расставь флот, стреляй по скрытой доске и забери двойной банк.', action: 'В море' }, { id: 'mines', icon: '💣', eyebrow: 'PIGGY MINES', title: 'Свиные мины', text: 'Открывай клетки, избегай бомб и забирай множитель когда захочешь.', action: 'К минам' }, { id: 'road', icon: '🐷', eyebrow: 'PIGGY ROAD', title: 'Свиная дорога', text: 'Прыгай по платформам, забирай множитель и не попадай на мину.', action: 'К дороге' }, { id: 'contract', icon: '📜', eyebrow: 'PIGGY CONTRACT', title: 'Контракт', text: 'Объедини 3–10 скинов и получи один новый предмет.', action: 'К контракту' }, { id: 'crash', icon: '🚀', eyebrow: 'PIGGY CRASH', title: 'Свинокраш', text: 'Поставь до старта и успей забрать множитель до краша.', action: 'К крашу' }]; return <section className="page compact-page games-hub"><div className="page-title"><p className="eyebrow">SVINODROP GAME ROOM</p><h1>Выбери свою <strong>игру</strong></h1><p>Семь режимов, один свинобаланс и настоящая конкуренция с игроками.</p></div><div className="games-grid">{games.map((game) => <button className={`game-hub-card ${game.id}`} key={game.id} onClick={() => onOpen(game.id)}><span>{game.icon}</span><p>{game.eyebrow}</p><h2>{game.title}</h2><small>{game.text}</small><b>{game.action} <em>→</em></b></button>)}</div></section> }
...
function NavalPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  ...
  const shot = async (x: number, y: number) => { if (!current) return; try { const next = await request(`/api/naval/${current.id}/shot`, token, { method: 'POST', body: JSON.stringify({ x, y }) }); const last = next.opponent?.shots?.filter((entry: NavalShot) => !entry.blocked).at(-1); playSiteSound(last?.sunk ? 'win' : last?.hit ? 'hit' : 'shot'); setCurrent(next); await onBalance(); load() } catch (error) { toast(error instanceof Error ? error.message : 'Выстрел не прошёл') } };
...
function OnboardingModal({ username, onClose, onStart }: { username: string; onClose: () => void; onStart: () => void }) {
  const [step, setStep] = useState(0)
  const slides = [
    { icon: '🐷', eyebrow: 'ДОБРО ПОЖАЛОВАТЬ В СТАЮ', title: `Привет, ${username}!`, text: 'Здесь всё виртуально: открывай кейсы, собирай скины и прокачивай свой свинобаланс в удобном темпе.', points: ['Стартовый баланс уже ждёт в профиле', 'Все полученные скины лежат в инвентаре'] },
    { icon: '📦', eyebrow: 'КЕЙСЫ И ИНВЕНТАРЬ', title: 'Начни с открытия', text: 'Выбирай кейс, смотри шансы и открывай по одному или серией. Выпавший предмет можно оставить, продать или использовать в следующем режиме.', points: ['♥ Добавляй любимые кейсы в избранное', 'Поиск поможет быстро найти нужный кейс'] },
    { icon: '🎮', eyebrow: 'ИГРОВАЯ КОМНАТА', title: 'Шесть режимов риска', text: 'Апгрейд, баттлы, морской бой, мины, контракт и краш — каждый режим подробно объясняет правила прямо на своей странице.', points: ['Не ставь больше, чем готов потерять', 'В играх результат фиксируется сервером'] },
    { icon: '💳', eyebrow: 'БАЛАНС И ВОЗМОЖНОСТИ', title: 'Баланс под контролем', text: 'В профиле есть ежедневный кейс и свинокредит: можно получать до 150 000 SC в день частями. Там же меняются аватар, цвет ника и видна история апгрейдов.', points: ['Кнопка «Пополнить баланс» ведёт в профиль', '🔊 в шапке включает или выключает игровые звуки'] },
  ]
  const current = slides[step]
  const last = step === slides.length - 1
  return <div className="modal-backdrop onboarding-backdrop"><section className="onboarding-modal" aria-label="Знакомство с сайтом"><button className="close" onClick={onClose}>×</button><div className="onboarding-art"><span>{current.icon}</span><i>✦</i><i>✦</i><i>✦</i></div><p className="eyebrow">{current.eyebrow}</p><h2>{current.title}</h2><p className="onboarding-copy">{current.text}</p><ul>{current.points.map((point) => <li key={point}><span>✓</span>{point}</li>)}</ul><div className="onboarding-footer"><div className="onboarding-dots">{slides.map((_, index) => <button key={index} aria-label={`Шаг ${index + 1}`} className={index === step ? 'active' : ''} onClick={() => setStep(index)}/>)}</div><div>{step > 0 && <button className="ghost-button onboarding-back" onClick={() => setStep((value) => value - 1)}>Назад</button>}<button className="pig-button" onClick={() => last ? onStart() : setStep((value) => value + 1)}>{last ? 'К КЕЙСАМ →' : 'ДАЛЬШЕ →'}</button></div></div><button className="text-button onboarding-skip" onClick={onClose}>Пропустить знакомство</button></section></div>
}
...
function RiskReveal({ pool, winner, phase, onFinished }: { pool: Skin[]; winner?: Skin; phase: 'idle' | 'spinning' | 'result'; onFinished: () => void }) {
  ...
  useEffect(() => {
    ...
    const timers = Array.from({ length: attempts }, (_, index) => window.setTimeout(() => { playSiteSound('card'); setRevealed(index + 1) }, (index + 1) * 980))
...
function AdminPanelV2({ token, toast }: { token: string; toast: (text: string) => void }) {
  ...
  const addItem = async () => { const id = window.prompt('Уникальный ID предмета:'); const name = window.prompt('Название предмета:'); const price = Number(window.prompt('Цена в свинокоинах:')); const image = window.prompt('Ссылка на изображение:'); const upgradeEligible = window.confirm('Разрешить использовать этот предмет как цель апгрейда?'); if (!id || !name || !price || !image) return; try { await request('/api/admin/items', token, { method: 'POST', body: JSON.stringify({ id, name, price: Math.round(price * 100), image, wear: 'FN', rarity: 'CLASSIFIED', upgradeEligible }) }); toast(upgradeEligible ? 'Предмет добавлен и доступен в апгрейдах' : 'Предмет добавлен без доступа в апгрейдах'); load() } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось добавить предмет') } }
  const editItem = async (item: { id: string; name: string; price: number; image: string; wear: string; rarity: string; upgradeEligible?: boolean }) => { const name = window.prompt('Название предмета:', item.name); const price = Number(window.prompt('Цена в свинокоинах:', String(item.price / 100))); const image = window.prompt('Ссылка на изображение:', item.image); const wear = window.prompt('Wear:', item.wear); const rarity = window.prompt('Редкость:', item.rarity); const upgradeEligible = window.confirm(`Разрешить «${item.name}» как цель апгрейда?`); if (!name || !price || !image || !wear || !rarity) return; try { await request(`/api/admin/items/${item.id}`, token, { method: 'PATCH', body: JSON.stringify({ name, price: Math.round(price * 100), image, wear, rarity, upgradeEligible }) }); toast(upgradeEligible ? 'Предмет сохранён и доступен в апгрейдах' : 'Предмет сохранён без доступа в апгрейдах'); load() } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось сохранить предмет') } }
  ...
  if (giveawayEditor) return <GiveawayWorkshop token={token} onClose={() => setGiveawayEditor(false)} onSaved={() => { setGiveawayEditor(false); load(); toast('Розыгрыш создан и уже виден игрокам') }} onError={toast}/>
...
function GiveawayWorkshop({ token, onClose, onSaved, onError }: { token: string; onClose: () => void; onSaved: () => void; onError: (message: string) => void }) {
  ...
  useEffect(() => { request('/api/admin/items', token).then((data) => { const active = data.filter((item: Skin & { active?: boolean }) => item.active !== false); setItems(active); setPrizeItemId((current) => current || active[0]?.id || '') }).catch(() => onError('Не удалось загрузить каталог призов')) }, [token])
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (title.trim().length < 3 || !prizeItemId || entryPrice < 100 || duration < 1) return onError('Заполни название, приз, цену от 1 SC и срок.')
    setSaving(true)
    try { await request('/api/admin/giveaways', token, { method: 'POST', body: JSON.stringify({ title: title.trim(), prizeItemId, entryPrice, startsAt: new Date().toISOString(), endsAt: endsAt.toISOString() }) }); onSaved() } catch (error) { onError(error instanceof Error ? error.message : 'Не удалось создать розыгрыш') } finally { setSaving(false) }
  }
  return <section className="page compact-page giveaway-workshop-page"><form className="giveaway-workshop" onSubmit={submit}><header><div><p className="eyebrow">КОНСТРУКТОР РОЗЫГРЫШЕЙ</p><h1>Новый <strong>розыгрыш</strong></h1><p>Три простых шага: выбери приз, назначь цену билета и срок. Розыгрыш стартует сразу после сохранения.</p></div><button type="button" className="login" onClick={onClose}>← К розыгрышам</button></header><div className="giveaway-steps"><span className="active"><b>1</b> Приз</span><i/><span className="active"><b>2</b> Условия</span><i/><span className="active"><b>3</b> Готово</span></div><div className="giveaway-workshop-grid"><section><label>Название розыгрыша<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Например: Пятничный свинодроп"/></label><label>Цена билета, SC<input type="number" min="1" value={entryCoins} onChange={(event) => setEntryCoins(event.target.value)}/><small>Каждый игрок может купить один билет.</small></label><div className="giveaway-duration"><b>Длительность</b><div>{[[1,'1 час'],[24,'24 часа'],[72,'3 дня'],[168,'7 дней']].map(([hours,label]) => <button type="button" className={duration === hours ? 'chosen' : ''} key={hours} onClick={() => setDuration(hours as number)}>{label}</button>)}</div><label>Свой срок в часах<input type="number" min="1" max="720" value={duration} onChange={(event) => setDuration(Math.max(1, Number(event.target.value) || 1))}/></label></div></section><section className="giveaway-prize-picker"><div><b>Выбери приз</b><small>Только активные предметы из каталога.</small></div><div className="giveaway-prize-list">{items.map((item) => <button type="button" className={item.id === prizeItemId ? 'picked' : ''} key={item.id} onClick={() => setPrizeItemId(item.id)}><img src={item.image} alt=""/><span><b>{item.name}</b><small>{item.wear} · {coins(item.price)} SC</small></span></button>)}</div></section><aside className="giveaway-preview"><p className="eyebrow">ПРЕДПРОСМОТР</p>{prize ? <><img src={prize.image} alt=""/><small>ПРИЗ</small><b>{prize.name}</b><em>{coins(prize.price)} SC</em></> : <span>🎁</span>}<div><span>Билет <b>{entryCoins || 0} SC</b></span><span>Итоги <b>{endsAt.toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</b></span></div><button className="pig-button" disabled={saving || !prizeItemId}>{saving ? 'СОЗДАЁМ…' : 'ЗАПУСТИТЬ РОЗЫГРЫШ →'}</button></aside></div></form></section>
}
