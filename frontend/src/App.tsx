...
function BattleShowcase({ battle }: { battle: Battle }) {
      ...
      after(3600, () => {
        ...
        after(1150, () => {
          if (index + 1 < maxDrops) playRound(index + 1);
            ...
            if (battle.mode === 'JACKPOT') after(4600, () => setJackpotFinished(true));
          }
        });
      });
    };
    after(420, () => playRound(0));
  ...
  return <div className={`battle-showcase battle-case-window ${battle.mode === 'LAST' ? 'battle-last-mode' : ''}`}>
    ...
    <div className="battle-arena">{rounds.map((round, playerIndex) => {
      ...
      return <article className={`${finale && winnerKnown && playerIndex === results.winnerIndex ? 'winner' : ''} ${isOpening ? 'spinning' : ''} ${decisive ? 'decisive' : ''}`} key={`${round.player.username}-${playerIndex}`}>
        ...
        <div className="battle-case-reel" style={{ '--reel-duration': '3600ms' } as React.CSSProperties}>
    ...
    {stage === 'finale' && battle.mode === 'JACKPOT' && <div className={`battle-jackpot-finale ${jackpotFinished ? 'finished' : 'spinning'}`} aria-label="Финальный джекпот">
      <div className="battle-jackpot-head"><span>🎰</span><div><small>ФИНАЛЬНЫЙ РОЗЫГРЫШ</small><b>Джекпот банка</b></div><em>Шанс зависит от стоимости всех дропов</em></div>
...
function BattlePage({ token, cases, user, onRequireAuth, onBalance, toast }: { token: string; cases: Case[]; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  ...
  const create = async () => {
    ...
    setBusy(true);
    try { const created = await request('/api/battles', token, { method: 'POST', body: JSON.stringify({ caseIds: picked, mode, playerLimit: players, private: privateBattle }) }) as Battle; setPicked([]); setWatching(created); await onBalance(); load(); toast('Баттл создан — добавь бота или дождись игроков.') }
...
const fleetSizes = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]
...
function canPlaceShip(fleet: FleetShip[], candidate: FleetShip) { const cells = shipCells(candidate); if (cells.length !== candidate.size || cells.some((cell) => cell.x < 0 || cell.x > 9 || cell.y < 0 || cell.y > 9)) return false; const others = fleet.filter((ship) => ship.id !== candidate.id).flatMap(shipCells); return cells.every((cell) => others.every((occupied) => Math.abs(cell.x - occupied.x) > 1 || Math.abs(cell.y - occupied.y) > 1)) }
function randomFleet() { const fleet: FleetShip[] = []; for (const [index, size] of fleetSizes.entries()) { let placed = false; for (let attempt = 0; attempt < 700 && !placed; attempt += 1) { const vertical = size > 1 && Math.random() > .5; const candidate = { id: `fleet-${index}`, size, vertical, x: Math.floor(Math.random() * (vertical ? 10 : 11 - size)), y: Math.floor(Math.random() * (vertical ? 11 - size : 10)) }; if (canPlaceShip(fleet, candidate)) { fleet.push(candidate); placed = true } } } return fleet.length === fleetSizes.length ? fleet : emptyFleet() }
type PlacementHint = { x: number; y: number; valid: boolean }
function NavalBoard({ ships = [], shots = [], enemy, disabled, onCell, onDropCell, placementHints = [] }: { ships?: { x: number; y: number }[]; shots?: { x: number; y: number; hit: boolean; sunk?: boolean }[]; enemy?: boolean; disabled?: boolean; onCell?: (x: number, y: number) => void; onDropCell?: (x: number, y: number, shipId: string) => void; placementHints?: PlacementHint[] }) { return <div className={`naval-board ${enemy ? 'enemy' : ''}`}>{Array.from({ length: 100 }, (_, index) => { const x = index % 10; const y = Math.floor(index / 10); const ship = ships.some((cell) => cell.x === x && cell.y === y); const shot = shots.find((cell) => cell.x === x && cell.y === y); const hint = placementHints.find((cell) => cell.x === x && cell.y === y); return <button aria-label={hint ? `${String.fromCharCode(65 + y)}${x + 1}: ${hint.valid ? 'можно поставить' : 'нельзя поставить'}` : undefined} disabled={disabled || !!shot} className={`${ship && !enemy ? 'ship' : ''} ${hint?.valid ? 'place-ok' : hint ? 'place-no' : ''} ${shot?.hit ? 'hit' : shot ? 'miss' : ''} ${shot?.sunk ? 'sunk' : ''}`} key={`${x}:${y}`} onDragOver={(event) => { if (onDropCell) event.preventDefault() }} onDrop={(event) => { const shipId = event.dataTransfer.getData('text/plain'); if (shipId) onDropCell?.(x, y, shipId) }} onClick={() => onCell?.(x,y)}>{ship && !enemy && !shot ? <i>⚓</i> : shot?.hit ? '✹' : shot ? '·' : hint?.valid ? '•' : ''}</button> })}</div> }
function NavalPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  const [games, setGames] = useState<any[]>([]); const [current, setCurrent] = useState<NavalGame | null>(null); const [stake, setStake] = useState('100'); const [fleet, setFleet] = useState<FleetShip[]>(emptyFleet); const [selectedShipId, setSelectedShipId] = useState('fleet-0');
  ...
  const ready = async () => { if (!current) return; if (fleetCells(fleet).length !== 20) return toast('Сначала расставь все 20 клеток флота.'); try { setCurrent(await request(`/api/naval/${current.id}/ships`, token, { method: 'POST', body: JSON.stringify({ ships: fleetCells(fleet) }) })); load() } catch (error) { toast(error instanceof Error ? error.message : 'Проверь расстановку кораблей') } };
  ...
  const rotateShip = () => setFleet((currentFleet) => { const ship = currentFleet.find((entry) => entry.id === selectedShipId); if (!ship || ship.size === 1) return currentFleet; const vertical = !ship.vertical; if (ship.x < 0 || ship.y < 0) return currentFleet.map((entry) => entry.id === ship.id ? { ...entry, vertical } : entry); const candidate = { ...ship, vertical, x: Math.min(ship.x, vertical ? 9 : 10 - ship.size), y: Math.min(ship.y, vertical ? 10 - ship.size : 9) }; return canPlaceShip(currentFleet, candidate) ? currentFleet.map((entry) => entry.id === ship.id ? candidate : entry) : currentFleet });
  const lastShot = current?.opponent?.shots.length ? current.opponent.shots[current.opponent.shots.length - 1] : null; const selectedShip = fleet.find((ship) => ship.id === selectedShipId); const placementHints = current?.status === 'SETUP' && !current.mine.ready && selectedShip ? Array.from({ length: 100 }, (_, index) => { const x = index % 10; const y = Math.floor(index / 10); return { x, y, valid: canPlaceShip(fleet, { ...selectedShip, x, y }) } }) : [];
...
function RiskReveal({ pool, winner, phase, onFinished }: { pool: Skin[]; winner?: Skin; phase: 'idle' | 'spinning' | 'result'; onFinished: () => void }) {
  ...
  const previews = Array.from({ length: attempts }, (_, index) => index === attempts - 1 ? winner : pool[(seed + index * 9) % Math.max(pool.length, 1)] || winner)
  const pointerIndex = Math.min(revealed, attempts - 1)
  return <section className={`risk-reveal ${phase} ${winner ? 'ready' : 'waiting'}`} style={{ '--risk-columns': attempts, '--risk-pointer': `${(pointerIndex + .5) / attempts * 100}%` } as React.CSSProperties}>
    <div className="risk-reveal-head"><span>🎲</span><div><small>АЗАРТНАЯ ПРОВЕРКА</small><b>{winner ? `ВОПРОС ${Math.min(revealed + 1, attempts)} ИЗ ${attempts}` : 'СВИНОСЕРВЕР ПРЯЧЕТ ПРИЗ'}</b></div><em>Только последний вопрос — твой дроп</em></div>
    <div className="risk-question-reel"><i className="risk-pointer">▼</i><div className="risk-question-row">{previews.map((skin, index) => { const open = index < revealed; const final = index === attempts - 1; return <article className={`${open ? 'opened' : ''} ${index === pointerIndex && phase === 'spinning' ? 'active' : ''} ${final ? 'final' : ''}`} key={`${skin?.id || 'question'}-${index}`}>{open && skin ? <><img src={skin.image} alt=""/><small>{final ? 'ТВОЙ ПРИЗ' : 'ЛОЖНЫЙ СЛЕД'}</small></> : <><span>?</span><small>СКРЫТО</small></>}</article> })}</div></div>
    <p>{phase === 'spinning' ? 'Стрелка делает остановки на вопросах и постепенно раскрывает путь к финальному дропу.' : 'Финальный вопрос раскрыт — приз уже в твоём инвентаре.'}</p>
  </section>
}
