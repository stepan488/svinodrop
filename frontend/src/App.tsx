...
export default function App() {
  ...
  const finishCaseAnimation = async () => { if (casePhase !== 'spinning' || !opening) return; playSiteSound('reveal'); setCasePhase('result'); await Promise.all(opening.map((drop) => request(`/api/drops/${drop.dropId}/reveal`, token, { method: 'POST' }))); await refreshPrivate(); toast('Дроп уже в твоём инвентаре!') }
