...
type Case = { id: string; name: string; slug: string; image: string; price: number; collection: string; openingStyle?: 'REEL' | 'MAGIC'; maxOpen?: number; contentsHidden?: boolean; items: { id: string; weight: number; item: Skin }[] }
...
type Battle = { id: string; status: string; mode: 'NORMAL' | 'CURSED' | 'JACKPOT' | 'LAST'; playerLimit: number; private: boolean; inviteCode: string; creatorId: string; players: { id: string; userId: string | null; username: string; avatar: string | null }[]; cases: Pick<Case, 'id' | 'name' | 'image' | 'price'>[]; results?: { rounds: { player: { username: string; avatar?: string | null }; drops: { caseId?: string; caseName?: string; item: Skin; value: number }[]; total: number }[]; winnerIndex: number }; winnerUserId?: string | null; isMine: boolean }
type NavalGame = { id: string; stake: number; status: string; turnUserId: string | null; winnerUserId: string | null; mine: { ready: boolean; ships: { x: number; y: number }[]; shots: { x: number; y: number; hit: boolean; sunk?: boolean }[] }; opponent: { username: string; avatar: string | null; ready: boolean; shots: { x: number; y: number; hit: boolean; sunk?: boolean }[] } | null }
...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {page === 'battles' && <BattlePage token={token} cases={cases} user={user} onRequireAuth={() => setAuthOpen(true)} onBalance={refreshPrivate} toast={toast}/>}
...
function BattleShowcase({ battle }: { battle: Battle }) {
  ...
  useEffect(() => {
    setRevealed(0);
    const timer = window.setInterval(() => setRevealed((current) => current >= maxDrops ? current : current + 1), 650);
    return () => window.clearInterval(timer);
  }, [battle.id, maxDrops]);
  const jackpotTotal = rounds.reduce((sum, round) => sum + Math.max(1, round.total), 0);
  return <div className="battle-showcase">
    {battle.mode === 'JACKPOT' && <div className="jackpot-stage" aria-label="Рулетка джекпота">
      <i className="jackpot-pointer">▼</i>
      <div className="jackpot-track">
        {[...rounds, ...rounds].map((round, index) => {
          const percent = Math.max(1, round.total) / jackpotTotal * 100;
          const isWinner = index % rounds.length === results.winnerIndex;
          return <span className={isWinner ? 'winner' : ''} style={{ flexGrow: Math.max(1, round.total) }} key={`${round.player.username}-${index}`} title={`${round.player.username}: ${percent.toFixed(1)}%`}><b>{round.player.avatar || '🐷'}</b><small>{percent.toFixed(0)}%</small></span>
        })}
      </div>
      <small>Рулетка выбрала победителя по доле каждого игрока в банке</small>
    </div>}
    <div className="battle-showcase-players">{rounds.map((round, playerIndex) => <article className={playerIndex === results.winnerIndex ? 'winner' : ''} key={`${round.player.username}-${playerIndex}`}>
      <header><span>{round.player.avatar || '🐷'}</span><b>{round.player.username}</b><strong>{coins(round.total)} SC</strong></header>
      <div className="battle-reel">{round.drops.map((drop, dropIndex) => <div className={`battle-drop ${dropIndex < revealed ? 'revealed' : 'rolling'}`} key={`${drop.item.id}-${dropIndex}`}>
        {dropIndex < revealed ? <><img src={drop.item.image} alt=""/><b>{drop.item.name}</b><em>{coins(drop.value)} SC</em></> : <><span>🎲</span><small>крутится кейс</small></>}
      </div>)}</div>
      {playerIndex === results.winnerIndex && <footer>🏆 Забирает весь дроп</footer>}
    </article>)}</div>
  </div>
}
...
function BattlePage({ token, cases, user, onRequireAuth, onBalance, toast }: { token: string; cases: Case[]; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  ...
  const create = async () => {
    ...
    setBusy(true);
    try { await request('/api/battles', token, { method: 'POST', body: JSON.stringify({ caseIds: picked, mode, playerLimit: players, private: privateBattle }) }); setPicked([]); await onBalance(); load(); toast('Баттл создан — зови игроков или добавь бота.') }
    catch (error) { toast(error instanceof Error ? error.message : 'Не удалось создать баттл') }
    finally { setBusy(false) }
  };
  const action = async (id: string, actionName: 'join' | 'bot') => { if (!user) return onRequireAuth(); try { await request(`/api/battles/${id}/${actionName}`, token, { method: 'POST' }); await onBalance(); load() } catch (error) { toast(error instanceof Error ? error.message : 'Действие не выполнено') } };
  ...
  return <section className="page compact-page battle-page">
    ...
    <div className="battle-layout"><section className="battle-builder"><div className="battle-top"><h2>Создание баттла</h2><label className="private-toggle"><input type="checkbox" checked={privateBattle} onChange={(event) => setPrivateBattle(event.target.checked)}/> Приватный</label></div><div className="battle-mode-row">{(Object.keys(battleLabels) as Battle['mode'][]).map((entry) => <button key={entry} className={mode === entry ? 'chosen' : ''} onClick={() => setMode(entry)}><b>{battleLabels[entry]}</b><small>{entry === 'NORMAL' ? 'Больше — победа' : entry === 'CURSED' ? 'Меньше — победа' : entry === 'JACKPOT' ? 'Шанс по сумме' : 'Решает последний'}</small></button>)}</div><div className="battle-player-row"><span>Игроков</span>{[2,3,4].map((count) => <button className={players === count ? 'chosen' : ''} key={count} onClick={() => setPlayers(count)}>{count}</button>)}<b>Вход: {coins(selectedCost)} SC</b></div><div className="battle-case-picker">{cases.slice(0,24).map((item) => { const count = picked.filter((id) => id === item.id).length; return <button key={item.id} className={count ? 'picked' : ''} onClick={() => setPicked((current) => current.length < 12 ? [...current, item.id] : current)}><img src={item.image} alt=""/>{count > 0 && <i>×{count}</i>}<span>{item.name}</span><b>{coins(item.price)} SC</b></button> })}</div><div className="battle-selected-cases">{picked.map((id, index) => { const item = cases.find((entry) => entry.id === id); return item ? <button key={`${id}-${index}`} title="Убрать этот раунд" onClick={() => setPicked((current) => current.filter((_, itemIndex) => itemIndex !== index))}><img src={item.image} alt=""/><span>{index + 1}</span><b>×</b></button> : null })}</div><div className="battle-builder-footer"><small>Добавлено кейсов: {picked.length}/12 · нажми иконку кейса ещё раз, чтобы повторить его</small><button className="pig-button" disabled={busy || !picked.length} onClick={create}>{busy ? 'Создаём…' : 'Создать баттл →'}</button></div></section>
    <section className="battle-list"><div className="section-heading"><div><p className="eyebrow">LIVE BATTLES</p><h2>Комнаты</h2></div><span>{battles.length} активно</span></div>{battles.length ? battles.map((battle) => <article className="battle-room" key={battle.id}><div className="battle-room-head"><span>⚔</span><div><b>{battleLabels[battle.mode]} баттл</b><small>{battle.players.length}/{battle.playerLimit} игроков · {battle.private ? 'приватный' : 'публичный'}</small></div><em>{battle.status === 'FINISHED' ? 'ЗАВЕРШЁН' : 'ОЖИДАНИЕ'}</em></div><div className="battle-case-strip">{battle.cases.map((item, index) => <img key={`${item.id}-${index}`} src={item.image} title={`${index + 1}. ${item.name}`} />)}</div><div className="battle-players">{battle.players.map((player) => <span key={player.id}>{player.avatar || '🐷'} {player.username}</span>)}</div>{battle.results && <BattleShowcase battle={battle}/>} {battle.status === 'WAITING' && <div className="battle-actions">{!battle.isMine && <button className="pig-button" onClick={() => action(battle.id,'join')}>Войти</button>}{battle.isMine && battle.players.length < battle.playerLimit && <button className="login" onClick={() => action(battle.id,'bot')}>+ Свинобот</button>}{battle.isMine && battle.private && <small>Код: <b>{battle.inviteCode}</b></small>}</div>}</article>) : <div className="empty-feed">Пока нет баттлов — создай первый.</div>}</section></div>
  </section>
}

const defaultFleet = [{x:0,y:0},{x:0,y:1},{x:0,y:2},{x:0,y:3},{x:0,y:4},{x:2,y:0},{x:2,y:1},{x:2,y:2},{x:2,y:3},{x:4,y:0},{x:4,y:1},{x:4,y:2},{x:6,y:0},{x:6,y:1},{x:6,y:2},{x:8,y:0},{x:8,y:1}]
function randomFleet() {
  const ships: { x: number; y: number }[] = [];
  for (const size of [5, 4, 3, 3, 2]) {
    let placed = false;
    for (let attempt = 0; attempt < 300 && !placed; attempt += 1) {
      const vertical = Math.random() > .5;
      const x = Math.floor(Math.random() * (vertical ? 10 : 11 - size));
      const y = Math.floor(Math.random() * (vertical ? 11 - size : 10));
      const candidate = Array.from({ length: size }, (_, index) => ({ x: x + (vertical ? 0 : index), y: y + (vertical ? index : 0) }));
      const clear = candidate.every((cell) => ships.every((occupied) => Math.abs(cell.x - occupied.x) > 1 || Math.abs(cell.y - occupied.y) > 1));
      if (clear) { ships.push(...candidate); placed = true; }
    }
  }
  return ships.length === 17 ? ships : defaultFleet;
}
function NavalBoard({ ships = [], shots = [], enemy, disabled, onCell }: { ships?: { x: number; y: number }[]; shots?: { x: number; y: number; hit: boolean; sunk?: boolean }[]; enemy?: boolean; disabled?: boolean; onCell?: (x: number, y: number) => void }) { return <div className={`naval-board ${enemy ? 'enemy' : ''}`}>{Array.from({ length: 100 }, (_, index) => { const x = index % 10; const y = Math.floor(index / 10); const ship = ships.some((cell) => cell.x === x && cell.y === y); const shot = shots.find((cell) => cell.x === x && cell.y === y); return <button disabled={disabled || !!shot} className={`${ship && !enemy ? 'ship' : ''} ${shot?.hit ? 'hit' : shot ? 'miss' : ''} ${shot?.sunk ? 'sunk' : ''}`} key={`${x}:${y}`} onClick={() => onCell?.(x,y)}>{ship && !enemy && !shot ? <i>⚓</i> : shot?.hit ? '✹' : shot ? '·' : ''}</button> })}</div> }
function NavalPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  ...
  const create = async () => { if (!user) return onRequireAuth(); try { const data = await request('/api/naval', token, { method: 'POST', body: JSON.stringify({ stake: Math.round(Number(stake) * 100) }) }); setCurrent(data); await onBalance(); load() } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось создать игру') } };
  const join = async (id: string) => { if (!user) return onRequireAuth(); try { const data = await request(`/api/naval/${id}/join`, token, { method: 'POST' }); setCurrent(data); await onBalance(); load() } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось войти') } };
  const ready = async () => { if (!current) return; try { setCurrent(await request(`/api/naval/${current.id}/ships`, token, { method: 'POST', body: JSON.stringify({ ships: fleet }) })); load() } catch (error) { toast(error instanceof Error ? error.message : 'Проверь расстановку кораблей') } };
  ...
  const toggleFleet = (x: number, y: number) => setFleet((currentFleet) => currentFleet.some((cell) => cell.x === x && cell.y === y) ? currentFleet.filter((cell) => cell.x !== x || cell.y !== y) : currentFleet.length < 17 ? [...currentFleet, {x,y}] : currentFleet);
  const lastShot = current?.opponent?.shots.length ? current.opponent.shots[current.opponent.shots.length - 1] : null;
  return <section className="page compact-page naval-page"><div className="page-title left"><p className="eyebrow">PIGGY NAVAL · 1V1 ONLINE</p><h1>Морской <strong>бой</strong></h1><p>Поставь виртуальные SC, расставь настоящий флот скрытно и забери банк ×2.</p></div>{!current ? <div className="naval-lobby"><section className="naval-create"><h2>Создать игру</h2><label>Ставка, SC<input type="number" min="100" max="500000" value={stake} onChange={(event) => setStake(event.target.value)}/></label><small>Минимум 100 · максимум 500 000 SC. Победитель получает ×2.</small><button className="pig-button" onClick={create}>Создать стол →</button></section><section className="naval-rooms"><h2>Открытые столы</h2>{games.length ? games.map((game) => <article key={game.id}><span>⚓</span><div><b>{game.players?.[0]?.user?.username || 'Свинка'}</b><small>Ставка {coins(game.stake)} SC · ожидание соперника</small></div><button className="login" onClick={() => join(game.id)}>Войти</button></article>) : <div className="empty-feed">Свободных столов пока нет.</div>}</section></div> : <section className="naval-game"><div className="naval-status"><b>{current.status === 'WAITING' ? 'Ждём соперника…' : current.status === 'SETUP' ? 'Расставьте корабли' : current.status === 'FINISHED' ? current.winnerUserId === user?.id ? 'ПОБЕДА · банк зачислен!' : 'Игра завершена' : current.turnUserId === user?.id ? 'Твой ход' : 'Ход соперника'}</b><span>Банк: {coins(current.stake * 2)} SC</span><button className="login" onClick={() => setCurrent(null)}>К столам</button></div>{current.status === 'WAITING' ? <div className="empty-feed">Поделись столом — второй игрок появится здесь автоматически.</div> : <div className="naval-fields"><div><h3>Твой флот</h3><NavalBoard ships={current.mine.ready ? current.mine.ships : fleet} shots={current.mine.shots} disabled={current.mine.ready || current.status !== 'SETUP'} onCell={toggleFleet}/>{!current.mine.ready && <div className="fleet-actions"><button className="login" onClick={() => setFleet(defaultFleet)}>По уставу</button><button className="login" onClick={() => setFleet(randomFleet())}>↻ Перемешать</button><button className="pig-button" onClick={ready}>Готово · {fleet.length}/17</button></div>}<small className="naval-hint">Корабли: 5, 4, 3, 3 и 2 клетки. Можно переставлять клетки вручную.</small></div><div><h3>{current.opponent?.username || 'Соперник'}</h3><NavalBoard enemy shots={current.opponent?.shots || []} disabled={current.status !== 'PLAYING' || current.turnUserId !== user?.id} onCell={shot}/>{lastShot && <div className={`naval-shot-info ${lastShot.hit ? 'hit' : 'miss'}`}>{lastShot.sunk ? '💥 Корабль уничтожен!' : lastShot.hit ? '🎯 Попадание — стреляй ещё!' : '🌊 Мимо — ход соперника'}</div>}<small className="naval-hint">Корабли противника скрыты. ✹ — попадание, · — мимо.</small></div></div>}</section>}</section>
}
