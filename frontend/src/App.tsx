...
function UpgradeDial({ chance, phase, result, mode }: { chance: number; phase: 'idle' | 'spinning' | 'result'; result: { success: boolean; landingAngle: number } | null; mode: SpinMode }) {
  ...
  return <div className={`sd-upgrade ${phase} ${result ? 'ready' : ''} ${outcomeClass}`} style={{ '--success-size': `${safeChance * 3.6}deg`, '--needle-end': `${endAngle}deg`, '--motion-duration': `${spinDuration[mode]}ms` } as React.CSSProperties}>
    ...
    <div className="sd-success-ring" aria-hidden="true"><svg viewBox="0 0 338 338"><defs><linearGradient id="svino-success-arc" x1="0" x2="1"><stop offset="0" stopColor="#cf237c"/><stop offset=".5" stopColor="#ffb8dd"/><stop offset="1" stopColor="#ff3f9f"/></linearGradient></defs><path className="sd-success-arc-glow" d={successArcPath}/><path className="sd-success-arc-line" d={successArcPath}/></svg></div>
