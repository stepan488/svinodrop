...
function BattleShowcase({ battle }: { battle: Battle }) {
  ...
  const jackpotMs = fast ? 2100 : 4600
  useEffect(() => {
    setRoundIndex(0);
    setStage('spin');
    setFinale(false);
  ...
  return <div className={`battle-showcase battle-case-window ${battle.mode === 'LAST' ? 'battle-last-mode' : ''} ${fast ? 'battle-fast-mode' : ''}`}>
...
function RiskReveal({ pool, winner, phase, onFinished }: { pool: Skin[]; winner?: Skin; phase: 'idle' | 'spinning' | 'result'; onFinished: () => void }) {
  ...
  return <section className={`risk-reveal ${phase} ${winner ? 'ready' : 'waiting'}`} style={{ '--risk-columns': fogSlots, '--risk-pointer': `${(pointerIndex + .5) / fogSlots * 100}%` } as React.CSSProperties}>
    <div className="risk-reveal-head"><span>🎲</span><div><small>АЗАРТНАЯ ПРОВЕРКА</small><b>{winner ? revealed ? 'ТУМАН ОТКРЫВАЕТ СЛЕД' : 'СТРЕЛКА ИЩЕТ ПЕРВЫЙ СЛЕД' : 'СВИНОСЕРВЕР ПРЯЧЕТ ПРИЗ'}</b></div><em>Сколько остановок впереди — неизвестно</em></div>
    <div className="risk-question-reel"><i className="risk-pointer">▼</i><div className="risk-question-row">{previews.map((skin, index) => { const open = phase === 'result' || path.slice(0, revealed).includes(index); const final = index === finalIndex; const fog = !open && !path.slice(revealed).includes(index); return <article className={`${open ? 'opened' : ''} ${index === pointerIndex && phase === 'spinning' ? 'active' : ''} ${open && final ? 'final' : ''} ${fog ? 'fog' : ''}`} key={`${skin?.id || 'question'}-${index}`}>{open && skin ? <><img src={skin.image} alt=""/><small>{final ? 'ТВОЙ ПРИЗ' : 'ЛОЖНЫЙ СЛЕД'}</small></> : <><span>?</span><small>{fog ? 'ТУМАН' : 'СКРЫТО'}</small></>}</article> })}</div></div>
    <p>{phase === 'spinning' ? 'Следующая остановка может оказаться последней — маршрут скрыт в тумане.' : 'Барабан остановился: остальные ячейки раскрыты, а приз уже в твоём инвентаре.'}</p>
  </section>
}
