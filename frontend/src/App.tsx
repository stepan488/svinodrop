...
export default function App() {
  const [page, setPage] = useState<'cases' | 'upgrade' | 'giveaways' | 'inventory' | 'profile' | 'chat' | 'leaderboard' | 'admin'>('cases')
  ...
  return <main className="app-shell">
    ...
    <header className="topbar">
      ...
      <nav>{([['cases', 'Кейсы'], ['upgrade', 'Апгрейд'], ['giveaways', 'Розыгрыши'], ['leaderboard', 'Лидерборд'], ['inventory', 'Инвентарь'], ['chat', 'Чат'], ...(user?.role === 'ADMIN' ? [['admin', 'Админ-панель'] as const] : [])] as const).map(([id, label]) => <button key={id} className={page === id ? 'active' : ''} onClick={() => setPage(id)}>{label}</button>)}</nav>
