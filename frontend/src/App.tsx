...
export default function App() {
  const [page, setPage] = useState<'cases' | 'games' | 'upgrade' | 'battles' | 'naval' | 'mines' | 'giveaways' | 'inventory' | 'profile' | 'chat' | 'leaderboard' | 'admin'>('cases')
  ...
  const [profile, setProfile] = useState<{ stats: { opens: number; upgrades: number; itemCount: number }; transactions: { id: string; type: string; amount: number; description: string; createdAt: string }[]; upgradeHistory: UpgradeHistory[]; daily: { available: boolean; nextAt: string | null; maxValue: number } } | null>(null)
