...
function PigRoadPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => Promise<void>; toast: (text: string) => void }) {
  ...
  return <section className="page compact-page road-page"><header className="road-hero"><div><p className="eyebrow">PIGGY ROAD · СРЕДНИЙ РЕЖИМ</p><h1>Свиная <strong>дорога</strong></h1><p>Проведи свинью по платформам. Первый прыжок особенно опасен, дальше шанс безопасной платформы — 70%.</p></div><button onClick={() => setRulesOpen((open) => !open)}>Как играть? {rulesOpen ? '⌃' : '⌄'}</button></header>{rulesOpen && <div className="road-rules"><b>Только средняя сложность · до ×48.</b><span>Поставь 500–555 000 SC, выбери одну из четырёх платформ и забери выигрыш после любого безопасного хода. Первый шаг безопасен с шансом 42%, все следующие — 70%.</span></div>}<div className="road-game"><section className={`road-stage ${game?.status?.toLowerCase() || 'idle'}`}><div className="road-stage-top"><span>🐷 ПЛАТФОРМА {playing ? game.steps.length + 1 : game?.status === 'WON' ? game.maxSteps : 1}/{game?.maxSteps || 12}</span><strong>{playing ? 'ВЫБЕРИ СЛЕДУЮЩИЙ РЕЛЬС' : game?.status === 'LOST' ? 'СВИНЬЯ СНЯТА С ДИСТАНЦИИ' : game?.status === 'WON' ? 'ДОРОГА ПРОЙДЕНА!' : 'ДОРОГА ЖДЁТ СВИНЬЮ'}</strong></div><div className="road-tracks"><span/><span/><span/></div><div className="road-pig-lane"><img className="road-pig" src={roadPigImage} alt="Свинья на старте"/><div className="road-finish">🏁</div></div><div className="road-platforms" key={roundKey}>{roadPlatformLabels.map((label, index) => { const selected = resolvedChoice === index; const state = selected ? (game?.last?.safe ? 'safe' : 'mine') : ''; return <button disabled={!playing || busy} onClick={() => step(index)} className={`road-platform ${state}`} key={label}><span className="road-platform-grate">{selected ? game?.last?.safe ? '🐷' : '💣' : '✦'}</span><b>{label}</b><em>{multiplier(nextMultiplier)}</em></button> })}</div><div className="road-stage-footer"><span>Текущий множитель <b>{multiplier(game?.multiplier || 1)}</b></span><span>Возможный шаг <b>{multiplier(nextMultiplier)}</b></span></div></section><aside className="road-controls"><p className="eyebrow">СВИНОСТАВКА</p><h2>{playing ? 'Прыгай аккуратно' : game?.status === 'LOST' ? 'Попробовать снова' : game?.status === 'WON' ? 'Дорога покорена' : 'Подготовь забег'}</h2><label>Сумма ставки, SC<input type="number" min="500" max="555000" disabled={playing} value={stake} onChange={(event) => setStake(event.target.value)}/></label><div className="road-quick"><button disabled={playing} onClick={() => setStake('500')}>500</button><button disabled={playing} onClick={() => setStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 200))))}>50%</button><button disabled={playing} onClick={() => setStake(String(Math.min(555000, Math.floor((user?.balance || 0) / 100))))}>MAX</button></div><div className="road-odds"><span><i/>Первый шаг: 42%</span><span><i/>Дальше: 70%</span><strong>Максимум {multiplier(48)}</strong></div>{playing && game.steps.length > 0 && <div className="road-cashout-preview">Сейчас можно забрать <b>{coins(Math.floor(game.wager * game.multiplier))} SC</b></div>}<button className={`pig-button road-action ${playing ? 'cashout' : ''}`} disabled={busy || (playing && !game.steps.length)} onClick={playing ? cashout : start}>{busy ? 'СВИНЬЯ ДУМАЕТ…' : playing ? `ЗАБРАТЬ ${coins(Math.floor(game.wager * game.multiplier))} SC →` : `НАЧАТЬ ЗА ${coins(stakeCoins * 100)} SC →`}</button></aside></div></section>
}
