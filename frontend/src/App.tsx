...
function ProfileCustomizer({ user, token, onSaved, toast }: { user: User; token: string; onSaved: (user: User) => void; toast: (text: string) => void }) { const avatars = ['🐷','🐽','👑','🎰','⚔️','🦄','🐸','🦊','🐯','🦈','👾','🤖']; const [avatar, setAvatar] = useState(user.avatar || '🐷'); const [nickColor, setNickColor] = useState(user.nickColor || '#ffffff'); const save = async () => { try { const data = await request('/api/profile/customize', token, { method: 'PATCH', body: JSON.stringify({ avatar, nickColor }) }); onSaved(data.user); toast('Профиль обновлён') } catch (error) { toast(error instanceof Error ? error.message : 'Не удалось обновить профиль') } }; return <section className="page compact-page profile-customizer"><div className="panel"><p className="eyebrow">СВИНОСТИЛЬ</p><h2>Настрой профиль</h2><div className="avatar-gallery">{avatars.map((item) => <button className={avatar === item ? 'chosen' : ''} onClick={() => setAvatar(item)} key={item}>{item}</button>)}</div><label>Цвет ника <input type="color" value={nickColor} onChange={(event) => setNickColor(event.target.value)}/><b style={{ color: nickColor }}>{user.username}</b></label><button className="pig-button" onClick={save}>Сохранить стиль →</button></div></section> }
...
function NavalPage({ token, user, onRequireAuth, onBalance, toast }: { token: string; user: User | null; onRequireAuth: () => void; onBalance: () => void; toast: (text: string) => void }) {
  ...
  useEffect(() => { if (current?.id) { localStorage.setItem('svino-naval-game', current.id); setSavedGameId(current.id) } }, [current?.id]);
    ...
    {!current ? <div className="naval-lobby">
      ...
      {current.status === 'WAITING' ? <div className="empty-feed">Поделись столом — второй игрок появится здесь автоматически.</div> : <div className="naval-fields">
        <div><h3>Твой флот</h3>
          ...
          {current.mine.shots.length > 0 && <div className={`naval-shot-info ${current.mine.shots.at(-1)?.hit ? 'hit' : 'miss'}`}>{current.mine.shots.at(-1)?.sunk ? '💥 ТВОЙ КОРАБЛЬ УНИЧТОЖЕН' : current.mine.shots.at(-1)?.hit ? '🎯 СОПЕРНИК ПОПАЛ В ТВОЙ КОРАБЛЬ' : '🌊 СОПЕРНИК ПРОМАХНУЛСЯ'}</div>}<small className="naval-hint naval-placement-hint"><b>1.</b> Выбери корабль снизу. <b className="hint-green">Зелёные</b> клетки подходят, <b className="hint-red">розовые</b> — нет. <b>2.</b> Кликни по зелёной клетке или перетащи корабль; поворот — отдельной кнопкой.</small>
        </div>
