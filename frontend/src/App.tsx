...
export default function App() {
  ...
  const selectCase = (item: Case) => { if (!casesReady) return toast('Кейсы загружаются, одну секунду 🐷'); setSelectedCase(item); setCount(item.maxOpen === 1 ? 1 : Math.min(4, item.maxOpen || 4, count)); setOpening(null); setCaseBalanceReward(0) }
